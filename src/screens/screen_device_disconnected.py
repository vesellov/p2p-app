...
class DeviceDisconnectedScreen(screen.AppScreen):
    ...
    def on_enter(self, *args):
        if _Debug:
            print('DeviceDisconnectedScreen.on_enter', args)
    ...
    def on_leave(self, *args):
        if _Debug:
            print('DeviceDisconnectedScreen.on_leave', args)
    ...
    def do_process_health(self, *args, **kwargs):
        if _Debug:
            print('DeviceDisconnectedScreen.do_process_health', self.currently_active, args, kwargs)
        if not self.currently_active:
            return
    ...
    def do_connect(self, interval=None):
        if _Debug:
            print('DeviceDisconnectedScreen.do_connect', self.currently_active)
        if not self.currently_active:
            return
        if web_sock_remote.is_started():
            web_sock_remote.stop()
